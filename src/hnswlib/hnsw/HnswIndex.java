		...
		implements Index<TId, TVector, TItem, TDistance>
{
	...
	public boolean add(TItem item)
	{
		...
		AtomicLong visitedInIndex = new AtomicLong(0);
		AtomicLong hopsInIndex = new AtomicLong(0);
						...
						if (currObj != null)
						{
							...
							for (int level = Math.min(randomLevel, entryPointCopy.maxLevel()); level >= 0; level--)
							{
								PriorityQueue<NodeIdAndDistance<TDistance>> topCandidates = searchBaseLayer(currObj,
										item.vector(), efConstruction, level, visitedInIndex, hopsInIndex);
	 ...
	 */
	@Override
	public List<String> addJoin(TItem item, TDistance threshold, AtomicLong visitedVertices, AtomicLong hops)
	{
		if (item.dimensions() != dimensions)
		{
			throw new IllegalArgumentException("Item does not have dimensionality of : " + dimensions);
				...
				synchronized (lock)
				{
					synchronized (newNode)
					{
						...
						if (currObj != null)
						{
							...
							// new: search for similar pairs
							
							PriorityQueue<NodeIdAndDistance<TDistance>> rangeSet = new PriorityQueue<>();
							
							rangeSet = rangeSearchBaseLayer3(currObj, item.vector(), ef, 0, threshold, visitedVertices, hops);
							int id = (int) item.id();

							// convert to a list of results
							while (!rangeSet.isEmpty())
							{
								NodeIdAndDistance<TDistance> pair = rangeSet.poll();

								double distance = 1 - (double) pair.distance;

								String r = (id + "," + nodes.get(pair.nodeId).item.id() + "," + distance);
	 ...
	 */
	@Override
	public List<String> searchParallel(TItem item, TDistance threshold, AtomicLong visitedVertices, AtomicLong hops)
	{
		...
		//int k = ef;
		int k = (int) Math.round(ef + (((double)threshold) * ef) );

		//PriorityQueue<NodeIdAndDistance<TDistance>> topCandidates = rangeSearchBaseLayer2(currObj, item.vector(), k, 0, threshold); // with rangeSet
		PriorityQueue<NodeIdAndDistance<TDistance>> topCandidates = rangeSearchBaseLayer3(currObj, item.vector(), k, 0, threshold, visitedVertices, hops); // without rangeSet, use topCandidates
		...
		while (!topCandidates.isEmpty())
		{
			...
			int pairId = (int) nodes.get(pair.nodeId).item.id();
			if ( pairId >  id) // do not include duplicate pairs
			{
				double distance = 1 - (double) pair.distance;
	...
	public List<String> searchParallelTopK(TItem item, int k, AtomicLong visitedVertices, AtomicLong hops)
	{
	    if (entryPoint == null)
	    {
	        return Collections.emptyList();
	    ...
	    // top-k search on base layer (level 0)
	    PriorityQueue<NodeIdAndDistance<TDistance>> topCandidates =
	    		searchBaseLayer(currObj, item.vector(), k, 0, visitedVertices, hops);
		...
		while (!topCandidates.isEmpty())
		{
		    ...
		    //if (pairId > id) // avoid duplicate pairs
		    //{
		        double distance = 1 - (double) pair.distance;
		        String r = (id + "," + nodes.get(pair.nodeId).item.id() + "," + distance);
		        results.add(r);
	 ...
	 */
	@Override
	public List<SearchResult<TItem, TDistance>> findNearest(TVector destination, int k, AtomicLong visitedVertices, AtomicLong hops)
	{
		...
		PriorityQueue<NodeIdAndDistance<TDistance>> topCandidates = searchBaseLayer(currObj, destination,
				Math.max(ef, k), 0, visitedVertices, hops);
	 ...
	 */
	@Override
	// return only the total number of similar pairs and the similarity value of the least similar pair within the result
	public SearchResultTotal findNearestTotal(TVector destination, int k, AtomicLong visitedVertices, AtomicLong hops)
	{

		if (entryPoint == null)
		{
			return  new SearchResultTotal(0, 0, 0, 0);
		}
		...
		// get the total of pairs and the smallest similarity
		int totalPairs = topCandidates.size();
		NodeIdAndDistance<TDistance> nearestCandidate = topCandidates.peek();
		 
		SearchResultTotal srt = new SearchResultTotal(totalPairs,(double) nearestCandidate.distance, visitedVertices.get(), hops.get());
	...
	private PriorityQueue<NodeIdAndDistance<TDistance>> searchBaseLayer(Node<TItem> entryPointNode, TVector destination,
			int k, int layer, AtomicLong visitedVertices, AtomicLong hops)
	{
	...
	public List<SearchResult<TItem, TDistance>> rangeSearch(TVector destination, TDistance ths)
	{
		...
		// Heuristic for initial k
		int k = (int) Math.round(ef + (((double)ths) * ef) );
		
		PriorityQueue<NodeIdAndDistance<TDistance>> topCandidates = rangeSearchBaseLayer(currObj, destination, k, 0,
				ths);
		...
		while (lt(topCandidates.peek().distance, ths))
		{
			...
			k = k + (int) Math.round( (((double)ths) * k) / ((double)topCandidates.peek().distance) );
			
			topCandidates = rangeSearchBaseLayer(currObj, destination, k, 0, ths);
	...
	public List<SearchResult<TItem, TDistance>> rangeSearch2(TVector destination, TDistance ths)
	{
		...
		AtomicLong visitedVertices = null;
		AtomicLong hops = null;
		
		//PriorityQueue<NodeIdAndDistance<TDistance>> topCandidates = rangeSearchBaseLayer2(currObj, destination, k, 0, ths); // with rangeSet
		PriorityQueue<NodeIdAndDistance<TDistance>> topCandidates = rangeSearchBaseLayer3(currObj, destination, k, 0, ths, visitedVertices, hops); // without rangeSet, use topCandidates
				...
				synchronized (node)
				{
					...
					for (int i = 0; i < candidates.size(); i++)
					{
						...
						if (!visitedBitSet.contains(candidateId))
						{
							...
							{
								candidateDistance = distanceFunction.distance(destination, candidateNode.item.vector());
							} else
							{
	...
	// Use a third priority list (rangeCandidates) to store items above the threshold
			...
			TVector destination, int efs, int layer, TDistance ths)
	{
		...
		try
		{
			...
			while (!candidateSet.isEmpty())
			{
				...
				//if (gt(currentPair.distance, lowerBound))
				if (gt(currentPair.distance, lowerBound) && gt(lowerBound, ths))
				{
					//// replaced by gt(lowerBound, ths))
	...
	//Use the topCandidates list itself to store the items and, in the end, remove those that are not above the threshold
	// final rangeSearchBaseLayer
	private PriorityQueue<NodeIdAndDistance<TDistance>> rangeSearchBaseLayer3(Node<TItem> entryPointNode,
			TVector destination, int efs, int layer, TDistance ths, AtomicLong visitedVertices, AtomicLong hops)
	{
	...
	class ExactView implements Index<TId, TVector, TItem, TDistance>
	{
		...
		@Override
		public List<SearchResult<TItem, TDistance>> findNearest(TVector vector, int k, AtomicLong visitedVertices, AtomicLong hops)
		{
		...
		public List<String> addJoin(TItem item, TDistance threshold, AtomicLong visitedVertices, AtomicLong hops)
		{
			return HnswIndex.this.addJoin(item, threshold, visitedVertices, hops);
		}
		
		@Override
		public List<String> searchParallel(TItem item, TDistance threshold, AtomicLong visitedVertices, AtomicLong hops)
		{
			return HnswIndex.this.searchParallel(item, threshold, visitedVertices, hops);
		}
		
		@Override
		public List<String> searchParallelTopK(TItem item, int k, AtomicLong visitedVertices, AtomicLong hops) {
		    return HnswIndex.this.searchParallelTopK(item, k, visitedVertices, hops);
		}
		...
		@Override
		public SearchResultHops addAllJoin(Collection<TItem> items, TDistance threshold) throws InterruptedException
		{
			return HnswIndex.this.addAllJoin(items, threshold);
		}

		@Override
		public SearchResultHops addAllJoin(Collection<TItem> items, TDistance threshold, ProgressListener listener)
				throws InterruptedException
		{
			return HnswIndex.this.addAllJoin(items, threshold, listener);
		}

		@Override
		public SearchResultHops addAllJoin(Collection<TItem> items, TDistance threshold, int numThreads,
				ProgressListener listener, int progressUpdateInterval) throws InterruptedException
		{
			return HnswIndex.this.addAllJoin(items, threshold, numThreads, listener, progressUpdateInterval);
		}

		@Override
		public SearchResultTotal findNearestTotal(TVector vector, int k, AtomicLong visitedVertices, AtomicLong hops)
		{
